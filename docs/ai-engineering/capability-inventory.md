...
| Reply suggestion | `POST /v1/tickets/:id/suggest-reply` and `PATCH …/suggest-reply/:llmCallId/outcome` in `apps/api/src/routes/v1/tickets.ts`; prompts `packages/intelligence/prompts/reply-suggestion/{es,en}.md` | Free-text draft | Agent accepts, edits or rejects | `kairo-llm-feature` |
...
| Retrieval | RPCs `find_similar_tickets`, `find_relevant_kb`; endpoints `/similar`, `/related-history`, `/knowledge-context` in `tickets.ts` | Similar resolved tickets and relevant KB articles | Agent reads the panel | `kairo-llm-feature` step 6 |
...
| Observability | Langfuse generation in `classify.ts` and `embed.ts`; `logLlmCall` in `apps/api/src/lib/llm-logging.ts`; `recordDecision` in `apps/api/src/lib/decision-telemetry.ts`; guide `packages/intelligence/OBSERVABILITY.md` | `kairo-llm-feature` step 9, `kairo-ai-evaluation` → production signals |
